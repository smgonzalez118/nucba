...
const Tools = () => {
    ...
    async function getRecomend() {
        ...
        if (lastday === 1) {
            $lastday.current.textContent = "Nota: el precio de cierre corresponde al cierre anterior" }
        else {
            $lastday.current.textContent = ""
        }
        })
        .catch( function() { 
            $lastday.currenttextContent = "Ticker incorrecto o información financiera no disponible"
    ...
    <form className="search__form">
        <h2 className='title'> ANÁLISIS DE ACTIVOS DE RENTA VARIABLE (ACCIONES) </h2>
        <p> Busque un activo financiero por su ticker y le brindaremos un estado de situación del mismo</p>

        <input type="text" name="user" onBlur={getRecomend} id="ticker" ref={$ticker} placeholder="TICKER"/>
        <input type="button" onClick={getRecomend} value="BUSCAR" id="login__form-button"/>

        <div className="results">
            <div className="precio" id="precioActual" ref={$precio_actual}></div>
            <div className="precio" id="mediaMovil20d" ref={$mediaMovil20d}></div>
